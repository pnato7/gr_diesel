...
def entradas():
    ...
    for nota in notas:
        notas_data.append({
            ...
            'total_pecas': float(nota.total_pecas or 0),
            'created_at': nota.created_at.strftime('%Y-%m-%d') if nota.created_at else '',
            'servico': {
                'id': nota.servico.id,
                'descricao': nota.servico.descricao[:50] if nota.servico.descricao else '',
                'cliente': {
                    'nome': nota.servico.cliente.nome if nota.servico.cliente else 'N/A'
                }
            }
        })
    
    notas_json = json.dumps(notas_data)
    return render_template('entradas.html', notas=notas, notas_json=notas_json)
